...
const BasicTypography = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:2023/projects/${projetId}/tasks`,config);
  ...
  const onDragEnd = async (result) => {
    ...
    try {
      // Appelez votre endpoint pour obtenir l'ID du statut correspondant à l'état
  ...
  return (
    <Modal show={true} onHide={onClose}>
      ...
      <Modal.Body>
        ...
        <p>Date début: {formatDate(task.dateDebutT)}</p>
        <p>Date fin: {formatDate(task.dateFinT)}</p>
      </Modal.Body>
      <Modal.Footer>
...
TaskDetailsModal.propTypes = {
  task: PropTypes.shape({
    ...
    dateDebutT: PropTypes.string.isRequired,
    dateFinT: PropTypes.string.isRequired
  }).isRequired,
