...
const BootstrapTable = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:2023/users', config);
  ...
  const handleDelete = async (id) => {
    const confirmDelete = window.confirm("Êtes-vous sûr de vouloir supprimer ce employé ?");
    if (confirmDelete) {
    try {
                  ...
                  {users.length > 0 ? (
                    users.map((user, index) => (
                      <tr key={user._id}>
                        ...
                        <td>{user.__t === 'UserNormal' ? 'Employe normal' : 'Administrateur'}</td>
                        <td>
                          <Switch
